...
module Yookassa
  ...
  class InvalidCredentials < ApiModelBase
    # Тип объекта.
    attr_accessor :type
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Yookassa::InvalidCredentials` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Yookassa::InvalidCredentials`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      code_validator = EnumAttributeValidator.new('String', ["invalid_credentials"])
    ...
    def code=(code)
      validator = EnumAttributeValidator.new('String', ["invalid_credentials"])
    ...
    def hash
      [type, id, description, parameter, retry_after, code].hash
    end
