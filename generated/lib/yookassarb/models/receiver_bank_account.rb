  ...
  class ReceiverBankAccount < Receiver
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Yookassa::ReceiverBankAccount` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Yookassa::ReceiverBankAccount`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @account_number.to_s.length > 20
        invalid_properties.push('invalid value for "account_number", the character length must be smaller than or equal to 20.')
      end
    ...
    def valid?
      ...
      return false if @account_number.to_s.length > 20
      return false if @account_number !~ Regexp.new(/\d{20}/)
    ...
    def account_number=(account_number)
      ...
      if account_number.to_s.length > 20
        fail ArgumentError, 'invalid value for "account_number", the character length must be smaller than or equal to 20.'
      end
