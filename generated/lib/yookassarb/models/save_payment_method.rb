...
module Yookassa
  ...
  class SavePaymentMethod < ApiModelBase
    attr_accessor :type
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Yookassa::SavePaymentMethod` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Yookassa::SavePaymentMethod`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] saved Value to be assigned
    def saved=(saved)
      if saved.nil?
        fail ArgumentError, 'saved cannot be nil'
      end
    ...
    def hash
      [type, id, saved, status, holder, title, confirmation].hash
    end
