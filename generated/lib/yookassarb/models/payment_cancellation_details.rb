...
module Yookassa
  # Комментарий к статусу canceled: кто отменил платеж и по какой причине. Подробнее про неуспешные платежи: https://yookassa.ru/developers/payment-acceptance/after-the-payment/declined-payments
  class PaymentCancellationDetails < ApiModelBase
    # Участник процесса платежа, который принял решение об отмене транзакции. Может принимать значения yoo_money, payment_network и merchant. Подробнее про инициаторов отмены платежа: https://yookassa.ru/developers/payment-acceptance/after-the-payment/declined-payments#cancellation-details-party
    attr_accessor :party

    # Причина отмены платежа. Перечень и описание возможных значений: https://yookassa.ru/developers/payment-acceptance/after-the-payment/declined-payments#cancellation-details-reason
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Yookassa::PaymentCancellationDetails` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Yookassa::PaymentCancellationDetails`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def party=(party)
      validator = EnumAttributeValidator.new('String', ["yoo_money", "payment_network", "merchant"])
    ...
    def reason=(reason)
      validator = EnumAttributeValidator.new('String', ["3d_secure_failed", "call_issuer", "card_expired", "payment_method_limit_exceeded", "payment_method_restricted", "country_forbidden", "general_decline", "fraud_suspected", "identification_required", "insufficient_funds", "invalid_card_number", "invalid_csc", "issuer_unavailable", "canceled_by_merchant", "permission_revoked", "internal_timeout", "expired_on_confirmation", "expired_on_capture", "unsupported_mobile_operator", "deal_expired"])
