...
module Yookassa
  # Получатель платежа
  class PaymentOrderRecipientUtilities < ApiModelBase
    # Название получателя.
    attr_accessor :name
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Yookassa::PaymentOrderRecipientUtilities` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Yookassa::PaymentOrderRecipientUtilities`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @kpp.nil?
        invalid_properties.push('invalid value for "kpp", kpp cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @inn !~ Regexp.new(/[0-9]{10}/)
      return false if @kpp.nil?
      return false if @kpp !~ Regexp.new(/[0-9]{9}/)
