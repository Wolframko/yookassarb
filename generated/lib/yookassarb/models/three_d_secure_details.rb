...
module Yookassa
  ...
  class ThreeDSecureDetails < ApiModelBase
    # Отображение пользователю формы для прохождения аутентификации по 3‑D Secure. Возможные значения: true — ЮKassa отобразила пользователю форму, чтобы он мог пройти аутентификацию по 3‑D Secure; false — платеж проходил без аутентификации по 3‑D Secure.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Yookassa::ThreeDSecureDetails` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Yookassa::ThreeDSecureDetails`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @applied.nil?
        invalid_properties.push('invalid value for "applied", applied cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @applied.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] applied Value to be assigned
    def applied=(applied)
      if applied.nil?
        fail ArgumentError, 'applied cannot be nil'
      end
