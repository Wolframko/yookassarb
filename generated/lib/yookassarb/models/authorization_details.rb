...
module Yookassa
  # Данные об авторизации платежа при оплате банковской картой. Присутствуют только для этих способов оплаты: банковская карта, Mir Pay, SberPay, T-Pay.
  class AuthorizationDetails < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Yookassa::AuthorizationDetails` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Yookassa::AuthorizationDetails`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @three_d_secure.nil?
        invalid_properties.push('invalid value for "three_d_secure", three_d_secure cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @three_d_secure.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] three_d_secure Value to be assigned
    def three_d_secure=(three_d_secure)
      if three_d_secure.nil?
        fail ArgumentError, 'three_d_secure cannot be nil'
      end
    ...
    def hash
      [rrn, auth_code, three_d_secure].hash
    end
