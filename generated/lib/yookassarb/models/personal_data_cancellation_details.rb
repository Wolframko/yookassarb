...
module Yookassa
  ...
  class PersonalDataCancellationDetails < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Yookassa::PersonalDataCancellationDetails` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Yookassa::PersonalDataCancellationDetails`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      party_validator = EnumAttributeValidator.new('String', ["yoo_money"])
      return false unless party_validator.valid?(@party)
      return false if @reason.nil?
      reason_validator = EnumAttributeValidator.new('String', ["expired_by_timeout"])
    ...
    def party=(party)
      validator = EnumAttributeValidator.new('String', ["yoo_money"])
    ...
    def reason=(reason)
      validator = EnumAttributeValidator.new('String', ["expired_by_timeout"])
