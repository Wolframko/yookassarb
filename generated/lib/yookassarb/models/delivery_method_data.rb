...
module Yookassa
  # Данные о способе доставки счета пользователю. Подробнее о способах доставки счетов: https://yookassa.ru/developers/payment-acceptance/scenario-extensions/invoices/basics#send-invoice
  class DeliveryMethodData < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Yookassa::DeliveryMethodData` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Yookassa::DeliveryMethodData`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["self", "sms", "email"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["self", "sms", "email"])
