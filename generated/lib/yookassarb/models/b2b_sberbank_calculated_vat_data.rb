...
module Yookassa
  ...
  class B2bSberbankCalculatedVatData < B2bSberbankVatData
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Yookassa::B2bSberbankCalculatedVatData` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Yookassa::B2bSberbankCalculatedVatData`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      rate_validator = EnumAttributeValidator.new('String', ["5", "7", "10", "20", "22"])
      return false unless rate_validator.valid?(@rate)
    ...
    def rate=(rate)
      validator = EnumAttributeValidator.new('String', ["5", "7", "10", "20", "22"])
      unless validator.valid?(rate)
        fail ArgumentError, "invalid value for \"rate\", must be one of #{validator.allowable_values}."
      end
      @rate = rate
    end
