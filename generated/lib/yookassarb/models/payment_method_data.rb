...
module Yookassa
  # Данные для оплаты конкретным способом: https://yookassa.ru/developers/payment-acceptance/integration-scenarios/manual-integration/basics#integration-options (payment_method). Вы можете не передавать этот объект в запросе. В этом случае пользователь будет выбирать способ оплаты на стороне ЮKassa.
  class PaymentMethodData < ApiModelBase
    attr_accessor :type
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Yookassa::PaymentMethodData` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Yookassa::PaymentMethodData`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["bank_card", "cash", "sberbank", "tinkoff_bank", "yoo_money", "mobile_balance", "b2b_sberbank", "sbp", "sber_loan", "electronic_certificate", "sber_bnpl"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["bank_card", "cash", "sberbank", "tinkoff_bank", "yoo_money", "mobile_balance", "b2b_sberbank", "sbp", "sber_loan", "electronic_certificate", "sber_bnpl"])
