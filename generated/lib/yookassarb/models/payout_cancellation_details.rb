...
module Yookassa
  ...
  class PayoutCancellationDetails < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Yookassa::PayoutCancellationDetails` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Yookassa::PayoutCancellationDetails`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      party_validator = EnumAttributeValidator.new('String', ["yoo_money", "payout_network"])
      return false unless party_validator.valid?(@party)
      return false if @reason.nil?
      reason_validator = EnumAttributeValidator.new('String', ["insufficient_funds", "fraud_suspected", "one_time_limit_exceeded", "periodic_limit_exceeded", "rejected_by_payee", "general_decline", "issuer_unavailable", "recipient_not_found", "recipient_check_failed", "identification_required", "self_employed_annual_limit_exceeded"])
    ...
    def party=(party)
      validator = EnumAttributeValidator.new('String', ["yoo_money", "payout_network"])
    ...
    def reason=(reason)
      validator = EnumAttributeValidator.new('String', ["insufficient_funds", "fraud_suspected", "one_time_limit_exceeded", "periodic_limit_exceeded", "rejected_by_payee", "general_decline", "issuer_unavailable", "recipient_not_found", "recipient_check_failed", "identification_required", "self_employed_annual_limit_exceeded"])
