...
module Yookassa
  # Получатель платежа.
  class PaymentRecipient < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Yookassa::PaymentRecipient` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Yookassa::PaymentRecipient`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @gateway_id.nil?
        invalid_properties.push('invalid value for "gateway_id", gateway_id cannot be nil.')
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] gateway_id Value to be assigned
    def gateway_id=(gateway_id)
      if gateway_id.nil?
        fail ArgumentError, 'gateway_id cannot be nil'
      end
